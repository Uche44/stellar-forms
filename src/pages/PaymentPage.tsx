...
export const PaymentPage: React.FC = () => {
      ...
      <div className="min-h-screen bg-brand-bg flex items-center justify-center p-6 text-center">
        <div className="glass-panel p-8 rounded-2xl max-w-sm space-y-4">
          ...
          <p className="text-brand-textMuted text-xs leading-relaxed">
            This payment form might have been disabled, deleted, or the link is
            invalid. Please contact the creator.
          </p>
          <button
            onClick={() => navigate("/")}
            className="bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs px-4 py-2 rounded-xl transition-all"
          >
            Go Home
          </button>
        </div>
  ...
  const handlePayment = async (data: any) => {
    ...
    const loadingToast = toast.loading(
      "Initiating payment... please approve in wallet",
    );
    ...
    try {
      ...
      toast.loading("Submitting transaction to Stellar network...", {
        id: loadingToast,
      });
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex flex-col justify-between py-12 px-6">
      ...
      <div className="max-w-md w-full mx-auto glass-panel p-8 rounded-3xl relative z-10 space-y-6">
        <div className="flex items-center justify-between border-b border-white/5 pb-4">
          <div className="flex items-center gap-1 text-[10px] text-brand-textMuted font-bold uppercase tracking-wider">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-neon animate-pulse-slow" />{" "}
            Testnet Payment
          </div>
          <ShieldCheck
            size={18}
        ...
        >
          ...
          <div className="bg-brand-card/60 border border-white/5 p-5 rounded-2xl text-center">
            <span className="text-[10px] text-brand-textMuted font-bold uppercase tracking-wider block mb-2">
              Payment Amount
            </span>
          ...
          {form.customFields.length > 0 && (
            <div className="space-y-4">
              <span className="text-xs text-brand-textMuted font-semibold uppercase tracking-wider block">
          ...
          <div className="pt-2">
            {isConnected ? (
              ...
              >
                {isPaying
                  ? "Processing..."
                  : `Pay ${form.isVariableAmount ? "" : form.amount} XLM`}
              </button>
            ) : (
              <button
                type="button"
      ...
      <footer className="text-center text-[10px] text-brand-textMuted">
        StellarForms does not take custody of funds. XLM is settled
        peer-to-peer.
      </footer>
    </div>
  );
